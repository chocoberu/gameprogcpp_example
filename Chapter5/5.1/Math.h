...
namespace Math
{
	...
	inline float ToRadians(float degrees) // 도 -> 라디안
	{
		return degrees * Pi / 180.0f;
	}

	inline float ToDegrees(float radians) // 라디안 -> 도
	{
		return radians * 180.0f / Pi;
	}

	inline bool NearZero(float val, float epsilon = 0.001f) // epsilon 값과 비교해서 그보다 작으면 true 반환
	{
		if (fabs(val) <= epsilon)
		{
			return true;
