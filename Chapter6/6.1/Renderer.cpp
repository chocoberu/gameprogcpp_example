...
void Renderer::Shutdown()
{
	...
	//mMeshShader->Unload();
	mMapShader["Phong"]->Unload();
	mMapShader["BasicMesh"]->Unload();
	//delete mMeshShader;
	delete mMapShader["Phong"];
	delete mMapShader["BasicMesh"];
...
void Renderer::Draw()
{
	...
	{
		if(mc->GetMesh()->GetShaderName().compare("Phong") == 0)
		mc->Draw(mMapShader["Phong"]); // 메시 셰이더를 이용해서 그린다
	}

	// BasicMesh 셰이더 메시 컴포넌트 그리기
	mMapShader["BasicMesh"]->SetActive();
	mMapShader["BasicMesh"]->SetMatrixUniform("uViewProj", mView*mProjection);
	
	for (auto mc : mMeshComps)
	{
		if (mc->GetMesh()->GetShaderName().compare("BasicMesh") == 0)
			mc->Draw(mMapShader["BasicMesh"]);
	}
...
bool Renderer::LoadShaders()
{
	...
	mMapShader.emplace("Phong", mMeshShader);
	...
	temp->SetActive();
	temp->SetMatrixUniform("uViewProj", mView * mProjection);
	mMapShader.emplace("BasicMesh", temp);
	return true;
}
